...
//THIS IS THE SHARED MEMORY CODE
...
__global__ void conv_forward_kernel(float *output, const float *input, const float *mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    */
    ...
    #define sharedMem(i2, i1, i0) sharedMemory[(i2) * (shared_width * shared_width) + (i1) * shared_width + i0]
    ...
    for(int i = ty; i < shared_width; i += TILE_WIDTH) {
        for(int j = tx; j < shared_width; j += TILE_WIDTH) {
            if (heightBoundary + i < Height && widthBoundary + j < Width) {
                sharedMem(c_in, i, j) = in_4d(b_out, c_in, heightBoundary + i, widthBoundary + j);
            }  
        }
    }
    ...
    if (h < Height_out && w < Width_out) {
        ...
        for (int p = 0; p < K; p++) {
            for (int q = 0; q < K; q++)
            {
                result += sharedMem(c_in, ty + p, tx + q) * mask_4d(m_out, c_in, p, q);
            }
        }
        atomicAdd((&out_4d(b_out, m_out, h, w)), result);
    }
...
__host__ void GPUInterface::conv_forward_gpu_prolog(const float *host_output, const float *host_input, const float *host_mask, float **device_output_ptr, float **device_input_ptr, float **device_mask_ptr, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    cudaMemcpy(*device_mask_ptr, host_mask, Batch * Channel * Height * Width * sizeof(float), cudaMemcpyHostToDevice);
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    conv_forward_kernel<<<gridDim, blockDim, Channel * (TILE_WIDTH + K - 1) * (TILE_WIDTH + K - 1) * sizeof(float)>>>(device_output, device_input, device_mask, Batch, Map_out, Channel, Height, Width, K);
}
